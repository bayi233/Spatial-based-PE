...
import numpy as np
import math
from S_T.Transformer.strengthpos import StrengthenSpatialPosEncoding
...
class Encoder(nn.Module):
    ...
    def __init__(self, n_layers, d_k, d_v, d_model, d_ff, n_heads,
        ...
        self.shape_linear=nn.Linear(1536,d_model)
        self.activate=nn.GELU()
        self.inputnrom=nn.LayerNorm(d_model)
        self.inputdrop=nn.Dropout(dropout)


        # self.linear_pos=nn.Linear(2048,2048)
        ...
        self.pos=StrengthenSpatialPosEncoding(2048,23)
    def forward(self, enc_inputs,  return_attn=False):
        enc_inputs=enc_inputs.clone().detach().cuda()

        enc_inputs=self.shape_linear(enc_inputs)
        enc_inputs=self.activate(enc_inputs)
        enc_inputs=self.inputnrom(enc_inputs)
        enc_inputs=self.inputdrop(enc_inputs)




        batch=enc_inputs.shape[0]
        pos_size=int(math.sqrt(enc_inputs.shape[1]))
        pos=self.pos(batch,pos_size,pos_size).cuda()

        # pos=self.linear_pos(pos)
        # pos=self.pos_norm(pos)

        enc_outputs=enc_inputs+pos
